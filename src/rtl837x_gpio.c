...
static int rtl837x_gpio_request(struct gpio_chip *gc, unsigned int offset)
{
    ...
    uint32_t ret = gpio->priv->pMapper->gpio_muxSel_set(offset);
    if (ret)
    {
        dev_err(gpio->priv->dev, "gpio %d: failed to request gpio %x", offset, ret);
        return -EINVAL;
    }
    return 0;
}
...
static void rtl837x_gpio_set(struct gpio_chip *gc, unsigned offset, int value)
{
    ...
    uint32_t ret = gpio->priv->pMapper->gpio_pinVal_write(offset, val);
    if (ret)
    {
...
static int rtl837x_gpio_get(struct gpio_chip *gc, unsigned offset)
{
    ...
    uint32_t ret = gpio->priv->pMapper->gpio_pinVal_read(offset, &pval);
    if (ret)
        dev_err(gpio->priv->dev, "gpio %d: failed to read gpio val %x", offset, ret);
...
{
    struct rtl837x_gpio *gpio = gpiochip_get_data(gc);
    uint32_t ret = gpio->priv->pMapper->gpio_pinDir_set(offset, GPIO_DIR_INPUT);
    if (ret)
    {
        dev_err(gpio->priv->dev, "gpio %d: failed to set gpio direction %x", offset, ret);
        return -EINVAL;
    }
    return 0;
}
...
{
    struct rtl837x_gpio *gpio = gpiochip_get_data(gc);
    uint32_t ret = gpio->priv->pMapper->gpio_pinDir_set(offset, GPIO_DIR_OUTPUT);
    if (ret)
    {
        dev_err(gpio->priv->dev, "gpio %d: failed to set gpio direction %x", offset, ret);
        return -EINVAL;
    ...
    rtk_gpio_level_t val = value == 1 ? GPIO_LEVEL_HIGH : GPIO_LEVEL_LOW;
    ret = gpio->priv->pMapper->gpio_pinVal_write(offset, val);
    if (ret)
    {
        dev_err(gpio->priv->dev, "gpio %d: failed to write gpio val %x", offset, ret);
        return -EIO;
